function [xnew, lambda, pos,mu] = monqp(H,c,A,b,C)
% function [xnew, lambda, pos] = monqp(H,c,A,b,C)
%
% min 1/2  x' H x - c' x                 
%  x 
% contrainte   A' x = b                    
...
% cshelton, Novembre 17, 2010: pour cours CS229 a UC Riverside
%               reduction des numbres des parametres
%               s'il vous plait, voir l'originale a 
%          http://asi.insa-rouen.fr/enseignants/~arakotom/toolbox/index.html
% cshelton, Novembre 21, 2014: pour course CS229 a UC Riverside
%               change les evaluation Booleenne court-circuit
%               pour octave

% cshelton, November 17, 2010: for course CS229 at UC Riverside
%               reduction in the number of parameters
%               please look at the original at
%          http://asi.insa-rouen.fr/enseignants/~arakotom/toolbox/index.html
% cshelton, November 21, 2014: for course CS229 at UC Riverside
%               changed the short-circuit Boolean evaluations
%               for octave

% copyright subject to the GNU public license as per the included
% license.txt file
...
while STOP ~= 1
    ...
    if testchol==0
        ...
        M = At*(U\(U'\Ae));
        d = U\(U'\ce);
        d = (At*d - be);   % second membre
		% (homage au gars OlgaZZZZZZ qui nous a bien aid�)
    ...
    if (sum(xnew< 0) > 0 || sum(xnew > C(ind)) > 0)  &&  length(ind) > ncc  %  03/01/2002 AR
        ...
        xt = zeros(n,1);
        xt(ind) = xnew;
        xt(indsuptot) = C(indsuptot); indold=ind;
	   % calcul des multiplicateurs de lagrange associ�es aux contraintes:
        mu = H*xt - c + A*lambda;
	   % on ne regarde que les contraintes satur�es
        indsat = 1:n;
